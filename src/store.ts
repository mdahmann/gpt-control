...
const ProviderSchema = z.enum(["chrome_bridge", "codex", "responses", "oracle_browser", "oracle_api"]);
const RunStatusSchema = z.enum(["queued", "running", "completed", "failed", "cancelled", "needs_user"]);
const RecoverySchema = z.object({
	at: z.string(),
	action: z.enum(["reobserve", "reload", "restore_conversation_url", "retry", "continue"]),
	reason: z.string(),
...
const ReceiptSchema = z.object({
	...
	localBridgeSessionId: z.string().optional(),
...
const ConversationSchema = z.object({
	...
	bridgeTabId: z.number().int().nonnegative().optional(),
	bridgeAssistantTurnCount: z.number().int().nonnegative().optional(),
	workspaceRoot: z.string(),
...
export class RunStore {
	...
	async updateRun(id: string, update: Partial<RunRecord>): Promise<RunRecord> {
		...
		return this.withNamedLock(`record-${id}`, async () => {
			...
			if (TERMINAL.has(current.status) && requestedStatus !== undefined && requestedStatus !== current.status) return current;
