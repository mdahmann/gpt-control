...
export function passiveTransportDiscovery(env: NodeJS.ProcessEnv = process.env): Record<string, unknown> {
	...
	return {
		mode: "passive",
		chromeBridge: bridge
			? { installed: true, origin: bridge.origin, privateRequestTransport: Boolean(bridge.privateRpc), privateRequestOrigin: bridge.privateRpc?.origin }
			: { installed: false },
		codex: codex ? { installed: true, path: codex } : { installed: false },
		responses: { configured: Boolean(env.OPENAI_API_KEY), executionRequiresFreshOperatorConfirmation: true },
		oracle: oracle
		...
		note: "No discovered Bridge, Codex, or Oracle program was executed.",
	};
}
