...
export class FakeChromeBridge {
	...
	capabilities(): Capabilities {
		return { bridge: { launcher: this.launcher, probe: { ready: true, endpoint: "fake", extension: "connected" } } };
	}
	...
	private handleSession(args: string[]): ExecResult {
		...
		if (operation === "create") return ok({ sessionId: `fake-session-${this.nextSession++}` });
		...
		if (operation === "show") {
			...
			return ok({
				sessionId,
				name: this.options.foreignSession ? "other-tool" : "gpt-control:subagent:conv_fake",
				tabIds: [tab.id],
				state: tab.state,
			});
		}
	...
	private handleClick(tabId: number, selector: string): ExecResult {
		...
		if (selector === "text=Continue generating") return ok({ success: true });
...
export function makeChromeService(
	...
	const policy = operatorPolicyFromEnv({}, {
		...
		allowedTransports: ["chrome_bridge"],
